...
from friday import friday, Message, Config, get_collection, Filters

GBAN_USER_BASE = get_collection("GBAN_USER")
WHITELIST = get_collection("WHITELIST_USER")
GBAN_LOG = friday.getCLogger(__name__)
LOG = friday.getLogger(__name__)


async def is_admin(message: Message, me_id):
    check_user = await friday.get_chat_member(message.chat.id, me_id)
...
async def guadmin_check(chat_id, user_id) -> bool:
    check_status = await friday.get_chat_member(chat_id=chat_id, user_id=user_id)
...
async def gban_user(message: Message):
    ...
    get_mem = await friday.get_user_dict(user_id)
...
@friday.on_filters(~Filters.me & Filters.group & (Filters.text | Filters.new_chat_members))
