...
CHANNEL = friday.getCLogger(__name__)


async def is_admin(message: Message):
    check_user = await friday.get_chat_member(message.chat.id, message.from_user.id)
...
async def promote_usr(message: Message):
    ...
    get_group = await friday.get_chat(chat_id)
    ...
    if can_promo:
        ...
        if user_id:
            ...
            try:
                get_mem = await friday.get_chat_member(chat_id, user_id)
                await friday.promote_chat_member(chat_id, user_id,
                ...
                await friday.set_administrator_title(chat_id, user_id, custom_rank)
...
async def demote_usr(message: Message):
    ...
    if can_demote:
        ...
        if user_id:
            ...
            try:
                get_mem = await friday.get_chat_member(
                    chat_id,
                    message.reply_to_message.from_user.id
                    )
                await friday.promote_chat_member(chat_id, get_mem.user.id,
                                                 can_change_info=False,
                                                 can_delete_messages=False,
                                                 can_restrict_members=False,
                                                 can_invite_users=False,
...
async def ban_usr(message: Message):
    ...
    if can_ban:
        ...
        if user_id:
            ...
            try:
                get_mem = await friday.get_chat_member(chat_id, user_id)
                await friday.kick_chat_member(chat_id, user_id)
                await message.edit(
                    f"#BAN\n\n"
                    f"USER: [{get_mem.user.first_name}](tg://user?id={get_mem.user.id}) "
                    f"(`{get_mem.user.id}`)\n"
...
async def unban_usr(message: Message):
    ...
    if can_unban:
        ...
        if user_id:
            ...
            try:
                get_mem = await friday.get_chat_member(chat_id, user_id)
                await friday.unban_chat_member(chat_id, user_id)
            ...
            try:
                ...
                await friday.unban_chat_member(chat_id, get_mem.user.id)
                await message.edit("`🛡 Successfully Unbanned..`", del_in=0)
                await CHANNEL.log(
                    f"#UNBAN\n\n"
                    f"USER: [{get_mem.user.first_name}](tg://user?id={get_mem.user.id}) "
...
async def kick_usr(message: Message):
    ...
    if can_kick:
        ...
        if user_id:
            ...
            try:
                get_mem = await friday.get_chat_member(chat_id, user_id)
                await friday.kick_chat_member(chat_id, user_id, int(time.time() + 60))
                await message.edit(
                    f"#KICK\n\n"
                    f"USER: [{get_mem.user.first_name}](tg://user?id={get_mem.user.id}) "
                    f"(`{get_mem.user.id}`)\n"
            ...
            try:
                ...
                await friday.kick_chat_member(chat_id, get_mem.user.id, int(time.time() + 45))
                await message.edit(
                    f"#KICK\n\n"
                    f"USER: [{get_mem.user.first_name}](tg://user?id={get_mem.user.id}) "
                    f"(`{get_mem.user.id}`)\n"
    ...
    if can_mute:
        ...
        if minutes:
            ...
            try:
                ...
                await friday.restrict_chat_member(chat_id, user_id,
                                                  ChatPermissions(),
            ...
            try:
                get_mem = await friday.get_chat_member(chat_id, user_id)
                await friday.restrict_chat_member(chat_id, user_id, ChatPermissions())
                await message.edit(
                    f"#MUTE\n\n"
                    f"USER: [{get_mem.user.first_name}](tg://user?id={get_mem.user.id}) "
                    f"(`{get_mem.user.id}`)\n"
    ...
    if can_unmute:
        ...
        if user_id:
            ...
            try:
                ...
                await friday.restrict_chat_member(chat_id, get_mem.user.id,
                                                  ChatPermissions(
                                                      can_send_messages=amsg,
                                                      can_send_media_messages=amedia,
                                                      can_send_stickers=astickers,
...
async def zombie_clean(message: Message):
    ...
    if rm_delaccs:
        ...
        if can_clean:
            ...
            async for member in friday.iter_chat_members(chat_id):

                if member.user.is_deleted:

                    try:
                        await friday.kick_chat_member(
                            chat_id,
...
async def pin_msgs(message: Message):
    ...
    if can_pin:
        ...
        if unpin_pinned:
            ...
            try:
                await friday.unpin_chat_message(chat_id)
            ...
            try:
                ...
                await friday.pin_chat_message(chat_id, message_id, disable_notification=True)
                await message.delete()
                await CHANNEL.log(
                    f"#PIN-SILENT\n\n"
                    f"CHAT: `{get_group.title}` (`{chat_id}`)"
            ...
            try:
                message_id = message.reply_to_message.message_id
                await friday.pin_chat_message(chat_id, message_id)
                await message.delete()
                await CHANNEL.log(
                    f"#PIN\n\n"
                    f"CHAT: `{get_group.title}` (`{chat_id}`)"
        ...
        if gpic_set:
            ...
            if message.reply_to_message.photo:
                ...
                try:
                    ...
                    await friday.set_chat_photo(chat_id, img_id, img_ref)
                    await message.delete()
                    await CHANNEL.log(
                        f"#GPIC-SET\n\n"
                        f"CHAT: `{get_group.title}` (`{chat_id}`)"
                ...
                try:
                    gpic_path = await friday.download_media(message.reply_to_message)
                    await friday.set_chat_photo(message.chat.id, gpic_path)
                    await message.delete()
                    os.remove(gpic_path)
                    await CHANNEL.log(
                        f"#GPIC-SET\n\n"
            ...
            try:
                await friday.delete_chat_photo(chat_id)
                await message.delete()
                await CHANNEL.log(
                    f"#GPIC-DELETE\n\n"
                    f"CHAT: `{get_group.title}` (`{chat_id}`)"
...
async def smode_switch(message: Message):
    ...
    if can_do_smode:
        ...
        if seconds:
            try:
                seconds = int(seconds)
                await friday.set_slow_mode(chat_id, seconds)
                await message.edit(
            ...
            try:
                ...
                await friday.set_slow_mode(chat_id, smode_time)
                await message.edit(
            ...
            try:
                await friday.set_slow_mode(chat_id, 0)
