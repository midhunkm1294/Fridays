...
async def lock_perm(message: Message):
    ...
    get_perm = await friday.get_chat(chat_id)
    ...
    if lock_type == "all":
        try:
            await friday.set_chat_permissions(chat_id, ChatPermissions())
            await message.edit(
    ...
    try:
        await friday.set_chat_permissions(chat_id,
...
async def unlock_perm(message: Message):
    ...
    get_uperm = await friday.get_chat(chat_id)
...
async def view_perm(message: Message):
    ...
    v_perm = await friday.get_chat(message.chat.id)
    ...
    if v_perm is not None:
        try:
            ...
            if v_perm.photo:
                local_chat_photo = await friday.download_media(
                    message=v_perm.photo.big_file_id
                )

                await friday.send_photo(chat_id=message.chat.id,
