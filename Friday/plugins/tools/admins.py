...
async def mentionadmins(message: Message):
    ...
    try:
        async for x in friday.iter_chat_members(chat_id=chat_id, filter="administrators"):
            status = x.status
            u_id = x.user.id
            username = x.user.username or None
            full_name = (await friday.get_user_dict(u_id))['flname']
            if status == "creator":
