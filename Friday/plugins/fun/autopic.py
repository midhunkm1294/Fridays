...
from friday import friday, Message, Config, get_collection
...
async def autopic(message: Message):
    ...
    if os.path.exists(BASE_PIC) and not image_path:
        ...
        profile_photo = await friday.get_profile_photos("me", limit=1)
        if not profile_photo:
            await message.err("sorry, couldn't find any picture!")
            return
        await friday.download_media(profile_photo[0], file_name=BASE_PIC)
        store = True
    else:
...
async def apic_worker():
    user_dict = await friday.get_user_dict('me')
    ...
    while UPDATE_PIC:
        ...
        await friday.set_profile_photo(MDFY_PIC)
        os.remove(MDFY_PIC)
