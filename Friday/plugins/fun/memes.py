...
@friday.on_cmd("Oof$", about={'header': "Ooooof"}, trigger='')
...
@friday.on_cmd("Hmm$", about={'header': "Hmmmmm"}, trigger='')
...
@friday.on_cmd("clock$", about={'header': "kensar clock animation"})
...
async def slap_(message: Message):
    ...
    if not u_id:
        ...
        return
    info_dict = await friday.get_user_dict(u_id)
...
@friday.on_cmd("(yes|no|maybe|decide)$", about={
...
async def decide_(message: Message):
    ...
    await message.delete()
    await friday.send_photo(chat_id=chat_id,
                            photo=path,
    ...
    try:
        if scam_time > 0:
            ...
            while count <= scam_time:
                await friday.send_chat_action(chat_id, scam_action)
...
async def dice_gen(message: Message):
    ...
    await friday.send_dice(message.chat.id, random_emo)
    await message.delete()
